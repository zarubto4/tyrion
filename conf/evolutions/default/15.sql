...
create table hardwaregroup_tag (
  ...
  constraint pk_hardwaregroup_tag primary key (hardware_group_id,tag_id)
);

alter table hardwaregroup_tag add constraint fk_hardwaregroup_tag_hardwaregroup foreign key (hardware_group_id) references hardwaregroup (id) on delete restrict on update restrict;
create index ix_hardwaregroup_tag_hardwaregroup on hardwaregroup_tag (hardware_group_id);
